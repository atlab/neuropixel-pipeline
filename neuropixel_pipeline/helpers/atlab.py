...
@validate_call
def get_probe_serial_numbers(probe_calibration_dir:Path=PROBE_CALIBRATION_DIR) -> List[int]:
...
@validate_call
def probe_setup(probe_calibration_dir:Path=PROBE_CALIBRATION_DIR, insert=True) -> List[Dict[str, Any]]:
    """Requires datajoint access if insert is True (the default)"""

    probe_serial_nums = get_probe_serial_numbers(probe_calibration_dir=probe_calibration_dir)
    probes = [{
        'probe': serial_num,
